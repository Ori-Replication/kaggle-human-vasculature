...
from models.unet import unet_build_model  # we use unet for the model now
from models.fpn import fpn_build_model
...
def main():
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=16, num_workers=cfg.num_workers)
    val_dataset = KaggleDataset(cfg, mode='val')
    val_loader = DataLoader(val_dataset, batch_size=16,
                            num_workers=cfg.num_workers)

    model = unet_build_model(cfg)  # unet
    ...
    for epoch in tqdm(range(cfg.epochs)):
        train_one_epoch(model, train_loader, dice_loss, dice_loss, optimizer, scaler, scheduler, epoch, cfg)
        val(model, val_loader, dice_loss, dice_loss, epoch, cfg)
        model_dir = os.path.join(
            cfg.output_path, f'{cfg.model_name}_{date}_{time}_New_Unet')
...
def train_one_epoch(model, train_loader, loss_fn_1, loss_fn_2, optimizer, scaler, scheduler, epoch, cfg):
    ...
    for i, (images, masks) in enumerate(train_loader):
        ...
        images = norm_with_clip(
            images.reshape(-1, *images.shape[2:])).reshape(images.shape)
        ...
        iters.set_description(
            f"Epoch {epoch+1}/{cfg.epochs}, train loss: {train_loss:.4f}, dice score: {dice_score:.4f}")
        iters.update()
...
def val(model, val_loader, loss_fn_1, loss_fn_2, epoch, cfg):
    ...
    for i, (images, masks) in enumerate(val_loader):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                outputs = model(images)
                loss = 0.0 * loss_fn_1(outputs, masks) + \
                    1 * loss_fn_2(outputs, masks)
        val_loss = (val_loss*i + loss.item())/(i+1)
        val_dice_score = (val_dice_score*i +
                          surface_dice(outputs, masks))/(i+1)
        iters.set_description(
            f"Epoch {epoch+1}/{cfg.epochs}, val loss: {val_loss:.4f}, val dice score: {val_dice_score:.4f}")
        iters.update()
    iters.close()
